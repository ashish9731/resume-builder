...
create table if not exists credit_usage (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references auth.users(id) on delete cascade,
  service_type varchar(50) not null, -- 'resume_builder', 'resume_optimizer', 'communication_coach', 'interview_prep'
...
create policy "Admin full access to pricing tiers" on pricing_tiers
  for all using (auth.role() = 'service_role');

create policy "Admin full access to user tiers" on user_tiers
  for all using (auth.role() = 'service_role');

create policy "Admin full access to credit usage" on credit_usage
  for all using (auth.role() = 'service_role');
